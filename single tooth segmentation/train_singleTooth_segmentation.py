    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            volume_batch, label_batch, skeleton_batch, boundary_patch, kp_patch = (
                sampled_batch["image"],
                sampled_batch["label"],
                sampled_batch["skeleton"],
                sampled_batch["boundary"],
                sampled_batch["keypoints"],
            )
            volume_batch, label_batch, skeleton_batch, boundary_patch, kp_patch = (
                volume_batch.cuda(),
                label_batch.cuda(),
            ...
            loss_bd_dice = dice_loss(
                outputs_soft[:, 1, :, :, :], boundary_patch[:, :, :, :]
            )
            ...
            loss = (
                0.5 * (loss_seg_dice + loss_seg)
                + 0.1 * (loss_bd + loss_bd_dice)
                + 0.1 * (loss_kp + loss_kp_dice)
            )
