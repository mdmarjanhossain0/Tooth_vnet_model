...
parser.add_argument(
    "--exp",
    type=str,
    default="NC_1st_stage_cntV2_HZ_02_(1000data_256size_intensityClip)",
    help="model_name",
)
parser.add_argument(
    "--max_iterations", type=int, default=60000, help="maximum epoch number to train"
)
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            volume_batch, offset_batch, offset_skl_batch, label_batch = (
                sampled_batch["image"],
                sampled_batch["offset_cnt"],
                sampled_batch["offset_skl"],
                sampled_batch["label"],
            )
            volume_batch, offset_batch, offset_skl_batch, label_batch = (
                volume_batch.cuda(),
                offset_batch.cuda(),
                offset_skl_batch.cuda(),
                label_batch.cuda(),
            )
            ...
            loss_off = reg_criterion(
                outputs_off[:, :, label_batch[0, :, :, :] == 1],
                offset_batch[:, :, label_batch[0, :, :, :] == 1],
            )
            ...
            if iter_num % 600 == 0:
                ...
                writer.add_scalar(
                    "loss_test/test_loss", test_loss / iter_test, iter_num
                )
                net.train()
                del (
                    volume_batch,
