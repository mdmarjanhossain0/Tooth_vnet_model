...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        label = sample["label"]
        label = label.reshape(label.shape[0], label.shape[1], label.shape[2]).astype(
            np.float
        )
        return {
            "image": torch.from_numpy(image).float(),
            "offset_cnt": torch.from_numpy(offset_cnt).float(),
            "offset_skl": torch.from_numpy(offset_skl).float(),
...
class DataScale(object):
    ...
    def __call__(self, sample):
        image_t, label_t, offset_t, centroids_t = (
            sample["image"],
            sample["label"],
            sample["offset"],
            sample["centroids"],
        ...
        image_t = Variable(
            torch.from_numpy(image_t.astype(np.float)).type(torch.FloatTensor)
        )
        label_t = Variable(
            torch.from_numpy(label_t.astype(np.float)).type(torch.FloatTensor)
        )
        offset_t = Variable(
            torch.from_numpy(offset_t.astype(np.float)).type(torch.FloatTensor)
        )
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        image, offset_cnt, offset_skl, label = (
            sample["image"],
            sample["offset_cnt"],
            sample["offset_skl"],
            sample["label"],
